(function () {
    ...
    function renderResults(entries, query) {
        ...
        if (!tokens.length) {
            renderEmptyState("开始输入后，可搜索标题、标签、摘要和正文。", "search-status-empty");
            return;
        }

        if (!entries.length) {
            renderEmptyState("没有找到匹配的内容。", "search-status-empty");
            return;
        }

        status.textContent =
            "找到 " + entries.length + " 条与 “" + query + "” 相关的结果";
    ...
    if (!indexUrl) {
        renderEmptyState("搜索索引不可用，请确认搜索页 front matter 中包含 `outputs = [\"html\", \"json\"]`。", "search-status-error");
        return;
    }
    ...
    fetch(indexUrl)
        ...
        .catch(function () {
            renderEmptyState("当前页面的搜索索引加载失败。", "search-status-error");
        });
}());
