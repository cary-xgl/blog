...
如果把每一层理解成不同密度的索引，就有点像在做分层查找。层级越高，单次跨得越远，空间也会多一些，但整体还是 `O(n)`。

插入和删除时，不可能每次都硬维护出一套特别整齐的索引，那样代价太高。

Redis 用的是另一种做法：每插一个元素，就按概率决定它有多少层。数据少的时候，就算层级不太理想，影响也不大；数据多起来以后，整体上会慢慢接近一个比较均匀的跳表结构。Redis 默认概率是 `0.25`。

跳表相对红黑树有个好处，就是范围查找会更顺一些。其他像插入、删除这类场景，和红黑树差得不大。
...
```
Redis 默认会初始化 `0-9999` 这些整数字符串对象，后面要用时就直接加引用，这个范围也能配。
