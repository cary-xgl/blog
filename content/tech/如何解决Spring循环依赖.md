...
三级缓存不直接放对象，放的是一个延迟执行的 lambda。有人来取时才执行。
...
A 在填充属性时发现自己依赖 B，Spring 就转头去创建 B。
...
然后 B 又发现自己依赖 A。这时候 A 还没创建完，但它已经处在创建中了，所以 Spring 会去拿 A 的“早期引用”。
...
所以三级缓存解决的不是“有没有地方放”，而是“把提前暴露这件事推迟到真要用的时候再做”。
